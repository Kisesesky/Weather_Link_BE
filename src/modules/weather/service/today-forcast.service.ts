...
export class TodayForecastService {
  ...
  private async getForecastByRegion(
    nx: number,
    ny: number,
  ): Promise<ForecastTimeSlot[]> {
    ...
    try {
      ...
      const categories = ['TMP', 'SKY', 'POP', 'PTY', 'REH', 'SNO', 'WSD'];
      ...
      const filteredByDate = filtered.filter(
        (item) => item.fcstDate === today || item.fcstDate === tomorrow,
      );

      this.logger.log(
        `Filtered forecast items: ${filteredByDate.length} (from total ${forecastItems.length})`,
      );
      ...
      const groupedByTime = filteredByDate.reduce((acc, item) => {
        ...
        switch (item.category) {
          ...
          case 'POP':
            ...
            break;
          case 'PTY':
            acc[key].precipitationType =
              PrecipitationType[parseInt(item.fcstValue)];
            break;
          case 'REH':
          ...
          case 'SNO':
            ...
            break;
          case 'WSD':
            acc[key].windSpeed = parseFloat(item.fcstValue);
            break;
        }
  ...
  async collectAllRegionsWeather() {
    try {
      ...
      this.logger.log(
        `Starting to collect weather data for ${regions.length} regions`,
      );
      ...
      const forecastResults = await Promise.allSettled(
        regions.map(async (region) => {
          try {
            const forecasts = await this.getForecastByRegion(
              region.nx,
              region.ny,
            );
            return {
      ...
      if (failedRegions.length > 0) {
        this.logger.warn(
          `Failed to collect data for ${failedRegions.length} regions`,
        );
      ...
      this.logger.log(
        `Completed collecting weather data: ${JSON.stringify(result)}`,
      );
      return result;
    } catch (error) {
  ...
  async getForecastDataByRegionNameAfterCurrentTime(
    sido: string,
    gugun: string,
  ) {
    try {
      ...
      return {
        ...
        forecasts: forecasts.map((forecast) => ({
          ...
          rainType:
            PrecipitationType[
              this.getPrecipitationTypeCode(forecast.precipitationType)
            ],
        })),
      };
    } catch (error) {
      this.logger.error(
        `Error in getForecastDataByRegionNameAfterCurrentTime: ${error.message}`,
      );
      throw error;
    }
  }
